...
func (r *costBadgeRenderer) updateBackgroundColor() {
	...
	if variant == theme.VariantDark {
		...
		if r.badge.Cost == 0 {
			...
			bgColor = blendColors(cwstheme.CloudWorkstationColors.RunningDark, theme.BackgroundColor(), 0.3)
		} else if r.badge.Cost < 10 {
			// Medium cost - subtle orange
			bgColor = blendColors(cwstheme.CloudWorkstationColors.PendingDark, theme.BackgroundColor(), 0.3)
		} else {
			// High cost - subtle red
			bgColor = blendColors(cwstheme.CloudWorkstationColors.TerminatedDark, theme.BackgroundColor(), 0.3)
		}
	} else {
		// Light mode
		if r.badge.Cost == 0 {
			...
			// Low cost - subtle green
			bgColor = blendColors(cwstheme.CloudWorkstationColors.RunningLight, theme.BackgroundColor(), 0.2)
		} else if r.badge.Cost < 10 {
			// Medium cost - subtle orange
			bgColor = blendColors(cwstheme.CloudWorkstationColors.PendingLight, theme.BackgroundColor(), 0.2)
		} else {
			// High cost - subtle red
			bgColor = blendColors(cwstheme.CloudWorkstationColors.TerminatedLight, theme.BackgroundColor(), 0.2)
		}
	}
