...
// IdleCobraCommands handles idle/hibernation policy commands
type IdleCobraCommands struct {
	app *App
}

// NewIdleCobraCommands creates new idle cobra commands
func NewIdleCobraCommands(app *App) *IdleCobraCommands {
	return &IdleCobraCommands{app: app}
}

// CreateIdleCommand creates the idle command with subcommands
func (ic *IdleCobraCommands) CreateIdleCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "idle",
		Short: "Manage idle detection and hibernation policies",
		Long: `Configure idle detection policies to automatically hibernate or stop instances
when they're not in use, optimizing costs.`,
	}
	
	// Add subcommands
	cmd.AddCommand(
		ic.createProfileCommand(),
		ic.createInstanceCommand(),
		ic.createHistoryCommand(),
		ic.createStatusCommand(),
	)
	
	return cmd
}

// createProfileCommand creates the profile management subcommand
func (ic *IdleCobraCommands) createProfileCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "profile",
		Short: "Manage idle detection profiles",
		Long:  "Create, update, delete, and list idle detection profiles.",
	}
	...
	cmd.AddCommand(
		...
		&cobra.Command{
			Use:   "create <name>",
			Short: "Create a new idle profile",
		...
		&cobra.Command{
			Use:   "update <name>",
			Short: "Update an existing idle profile",
			Args:  cobra.ExactArgs(1),
			RunE: func(cmd *cobra.Command, args []string) error {
				// TODO: Implement
				return nil
	...
	createCmd.Flags().Int("idle-minutes", 30, "Minutes before marking as idle")
	createCmd.Flags().String("action", "hibernate", "Action to take (hibernate/stop)")
	createCmd.Flags().Float64("cpu-threshold", 10.0, "CPU usage threshold")
	createCmd.Flags().Float64("memory-threshold", 20.0, "Memory usage threshold")
	
	return cmd
}

// createInstanceCommand creates the instance idle configuration subcommand
func (ic *IdleCobraCommands) createInstanceCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "instance <name>",
		Short: "Configure idle policy for an instance",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
	...
	cmd.Flags().String("profile", "", "Idle profile to apply")
	cmd.Flags().Bool("disable", false, "Disable idle detection for this instance")
	
	return cmd
}

// createHistoryCommand creates the history viewing subcommand
func (ic *IdleCobraCommands) createHistoryCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "history",
		Short: "View idle detection history",
		RunE: func(cmd *cobra.Command, args []string) error {
	...
	cmd.Flags().String("instance", "", "Filter by instance name")
	cmd.Flags().Int("days", 7, "Number of days of history to show")
	
	return cmd
}

// createStatusCommand creates the status viewing subcommand
func (ic *IdleCobraCommands) createStatusCommand() *cobra.Command {
	return &cobra.Command{
		Use:   "status [instance]",
		Short: "Show idle detection status",
		RunE: func(cmd *cobra.Command, args []string) error {
			// TODO: Implement
			return nil
		},
