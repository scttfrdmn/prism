...
type LaunchRequest struct {
	...
	// Job array (spawn adoption): launch a group of related instances. When set,
	// spored discovers array peers (via the prism:job-array-id tag) and writes
	// /etc/spawn/job-array-peers.json for MPI / tightly-coupled workloads.
	// Populated per-member by Manager.LaunchArray; not typically set by a single-launch caller.
	JobArrayID    string `json:"job_array_id,omitempty"`    // Shared array id across members (generated by LaunchArray)
	JobArrayName  string `json:"job_array_name,omitempty"`  // User-facing array name
	JobArraySize  int    `json:"job_array_size,omitempty"`  // Total members in the array (N)
	JobArrayIndex int    `json:"job_array_index,omitempty"` // This member's index, 0..N-1

	// Parameter sweep (spawn adoption): launch one instance per parameter set. Each
	// member carries its own SweepParams, stamped as prism:param:<k> tags; an
	// on-instance shell (installed with spored) exports them as PARAM_<k> env vars.
	// Populated per-member by Manager.LaunchSweep; not typically set by a single caller.
	SweepID     string            `json:"sweep_id,omitempty"`     // Shared sweep id across members (generated by LaunchSweep)
	SweepName   string            `json:"sweep_name,omitempty"`   // User-facing sweep name
	SweepSize   int               `json:"sweep_size,omitempty"`   // Total parameter sets in the sweep (N)
	SweepIndex  int               `json:"sweep_index,omitempty"`  // This member's index, 0..N-1
	SweepParams map[string]string `json:"sweep_params,omitempty"` // This member's parameter set (key -> value)
}
...
// LaunchArrayRequest launches a job array: Count homogeneous instances sharing a
// job-array id, named <Name>-0..<Name>-(Count-1). The embedded LaunchRequest
// supplies the template/size/options common to every member.
type LaunchArrayRequest struct {
	LaunchRequest        // Name is the base name; per-member names are <Name>-<index>
	Count         int    `json:"count"`                    // Number of members (>= 1)
	JobArrayName  string `json:"job_array_name,omitempty"` // User-facing array name (defaults to Name)
}

// LaunchArrayResponse reports the outcome of a job-array launch. Members are
// independent: some may launch while others fail (partial success).
type LaunchArrayResponse struct {
	JobArrayID string     `json:"job_array_id"`
...
// LaunchSweepRequest launches a parameter sweep: one instance per parameter set,
// named <Name>-0..<Name>-(len(ParamSets)-1), all sharing a sweep id. The embedded
// LaunchRequest supplies the template/size/options common to every member; each
// member additionally gets its own parameter set from ParamSets[i]. The daemon
// does NOT parse parameter files — the client resolves ParamSets and sends them.
type LaunchSweepRequest struct {
	LaunchRequest                     // Name is the base name; per-member names are <Name>-<index>
	ParamSets     []map[string]string `json:"param_sets"`           // One parameter set per member (>= 1)
	SweepName     string              `json:"sweep_name,omitempty"` // User-facing sweep name (defaults to Name)
}

// LaunchSweepResponse reports the outcome of a parameter-sweep launch. Members are
// independent: some may launch while others fail (partial success).
type LaunchSweepResponse struct {
