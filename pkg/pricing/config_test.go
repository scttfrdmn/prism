...
func TestLoadInstitutionalPricing_Default(t *testing.T) {
	...
	defer func() {
		if oldEnv != "" {
			os.Setenv("PRICING_CONFIG", oldEnv)
		}
	}()
	...
	if data, err := os.ReadFile(currentConfig); err == nil {
		...
		os.Remove(currentConfig) // Temporarily remove
	}
	
	defer func() {
		if hadExistingConfig {
			os.WriteFile(currentConfig, backupData, 0644) // Restore
		}
	}()
...
func TestLoadInstitutionalPricing_FromFile(t *testing.T) {
	...
	oldEnv := os.Getenv("PRICING_CONFIG")
	os.Setenv("PRICING_CONFIG", configFile)
	defer os.Setenv("PRICING_CONFIG", oldEnv)
...
func TestLoadInstitutionalPricing_FileReadError(t *testing.T) {
	...
	oldEnv := os.Getenv("PRICING_CONFIG")
	os.Setenv("PRICING_CONFIG", "/nonexistent/path/config.json")
	defer func() {
		if oldEnv == "" {
			os.Unsetenv("PRICING_CONFIG")
		} else {
