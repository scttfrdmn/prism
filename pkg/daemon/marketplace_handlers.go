...
func getCurrentSystemUser() (userID, userName string) {
...
// getUserIDForRequest returns the current system user ID for API requests
func getUserIDForRequest() string {
...
func (s *Server) handleMarketplaceTemplateInstall(w http.ResponseWriter, r *http.Request) {
	...
	// Marketplace templates are standalone - they don't integrate with local template system
	// Users install them directly from the marketplace registry
